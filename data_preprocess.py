...
def save_spectrogram_spkr_reco(speaker_path, data_train_path):
...
def save_spectrogram_esc50(wavs_path, save_path):
    
    utter_min_len = (hp.data.tisv_frame * hp.data.hop + hp.data.window) * hp.data.sr    # lower bound of utterance length

    for utter_name in tqdm(os.listdir(wavs_path)):          
        utterances_spec = []
        utter_path = os.path.join(wavs_path, utter_name)         # path of each utterance
        ...
        for interval in intervals:
            if (interval[1]-interval[0]) > utter_min_len:           # If partial utterance is sufficient long
                ...
                # sliding window, window size = hp.data.tisv_frame, hop size = hp.data.tisv_frame//2
                for i in range(0, S.shape[1]-hp.data.tisv_frame, hp.data.tisv_frame//2):
                    utterances_spec.append(S[:, i:i+hp.data.tisv_frame])

        utterances_spec = np.array(utterances_spec)
        np.save(os.path.join(save_path, "{}.npy".format(utter_name.replace('.wav', ''))), utterances_spec)
