...
def get_mel_db_batch_corrupted_experiment2(x_batch, duplicate_num, percentage_chance):
    ...
    if(random.choice(chance_list)):
        ...
        mel_db_batch = []
        for i in range(len(x_batch) - duplicate_num):
            mel_db_batch.append(random.sample(split_tensor[i],1)[0])
        mel_db_batch.extend(random.sample(split_tensor[len(x_batch) - duplicate_num], duplicate_num))
