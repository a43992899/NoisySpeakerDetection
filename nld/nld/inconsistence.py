...
def compute_precision(ypreds: npt.NDArray, ylabels: npt.NDArray, noise_level: npt.NDArray):
    ...
    precision = selected_ylabels.sum() / len(selected_ylabels)
    return precision
...
def compute_distance_inconsistency(
    ...
    if not debug:
        wandb.init(
            ...
            entity=WANDB_ENTITY,
            name=f'distance-inconsistency-{train_config.description}',
...
def compute_confidence_inconsistency(
    ...
    if not debug:
        wandb.init(
            project=WANDB_NLD_PROJECT_NAME,
            entity=WANDB_ENTITY,
            name=f'conficence-inconsistency-{train_config.description}',
            config=nld_config.to_dict()
        )
