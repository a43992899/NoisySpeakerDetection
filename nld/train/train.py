...
def train(
    cfg: NewTrainConfig, mislabeled_json_file: Path, vox1_mel_spectrogram_dir: Path, 
    vox2_mel_spectrogram_dir: Path, training_model_save_dir: Path, enable_wandb: bool
):
    ...
    spkr2id_file = vox2_mel_spectrogram_dir / 'spkr2id.json'
    ...
    train_dataset = SpeakerDataset(1, vox1_mel_spectrogram_dir, vox2_mel_spectrogram_dir, mislabeled_json_file)
    ...
    for epoch in range(cfg.iterations):
        total_loss = 0
        for mels, is_noisy, ids, labels, _ in train_data_loader:
            ...
            mels = torch.reshape(
                mels,
                (cfg.N * cfg.M, mels.size(2), mels.size(3))
            )
