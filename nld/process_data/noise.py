...
from ..constant.config import Config as hp
...
def save_spectrogram_spkr_reco(data_path: str, output_path: str):
    ...
    for folder in tqdm(os.listdir(data_path)):
        ...
        for video in os.listdir(os.path.join(data_path, folder)):
            for utter_name in os.listdir(os.path.join(data_path, folder, video)):

                utter_path = os.path.join(data_path, folder, video, utter_name)         # path of each utterance
        ...
        np.save(os.path.join(output_path, "{}.npy".format(folder)), utterances_spec)
